...
def _frame_stats(frame: pd.DataFrame) -> dict[str, Any]:
    return {
        ...
        "label_distribution": {
            ...
            "malicious_1": int((frame["label"] == 1).sum()) if "label" in frame else 0,
        },
        "source_distribution": frame["source_dataset"].value_counts().to_dict() if "source_dataset" in frame else {},
    }
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    logger.info("Loaded %d rows across %d datasets", len(merged), len(summaries))
    for summary in summaries:
        logger.info("  - %s: %d rows (invalid=%d, dup=%d)", summary.dataset_name, summary.rows, summary.invalid_rows, summary.duplicate_rows)
    ...
    if args.max_samples_per_source > 0:
        ...
        logger.info("Sampled to %d rows (max_per_source=%d)", len(merged), args.max_samples_per_source)

    if "source" not in merged.columns:
        merged["source"] = merged.get("source_dataset", "unknown")

    logger.info("Starting evaluation pipeline with %d samples at threshold=%d", len(merged), args.threshold)
    ...
    if records.empty:
        ...
        return {"error": "Pipeline produced no results"}

    logger.info("Pipeline completed: %d successful, %d failures, %d timeouts", 
                runtime_summary.get("success_count", 0),
                runtime_summary.get("failure_count", 0),
                runtime_summary.get("timeout_exceeded_count", 0))
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help="Directory containing all datasets (auto-detects .txt, .csv, .json)"
    )
...
def main() -> None:
    ...
    if "error" not in report:
        ...
        logger.error("Benchmark failed: %s", report["error"])
